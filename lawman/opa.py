...
def evaluate_policy(intent: Intent, policy: Path, evidence: Evidence) -> Decision:
    ...
    document = {"intent": intent.to_dict(), "evidence": dict(evidence.facts)}
...
def _run(policy: Path, document: dict[str, Any]) -> Any:
    ...
    try:
        completed = subprocess.run(command, input=json.dumps(document), capture_output=True, text=True)
    ...
    if completed.returncode != 0:
        raise LawmanError(f"OPA could not evaluate {policy}: {_diagnostic(output, completed)}")
    if output is None:
        raise LawmanError(f"OPA output for {policy} is not valid JSON")
    return output


def _diagnostic(output: Any, completed: subprocess.CompletedProcess[str]) -> str:
    ...
    return stderr.splitlines()[0].strip() if stderr else f"exit status {completed.returncode}"
