...
from lawman import (Contract, ContractRegistry, Intent, LawmanError,
                    select_contract)
...
class FailsClosedWhenItCannotFindTheRules(unittest.TestCase):
    ...
    def test_a_registry_that_is_not_a_map_of_names_is_refused(self):
        for registry in ([], "deploy:production", {}, {"deploy:production": 7}, {"deploy": "c.json"}):
    ...
    def test_a_configured_contract_that_cannot_be_read_is_refused(self):
        with tempfile.TemporaryDirectory() as directory:
            configured = policy(directory, {"deploy:production": "contracts/deploy-production.json"})
    ...
    def test_a_configured_contract_that_is_not_valid_json_is_refused(self):
        with tempfile.TemporaryDirectory() as directory:
            configured = policy(
                directory,
                {"deploy:production": "contracts/deploy-production.json"},
                {"contracts/deploy-production.json": "{ not json"},
            )
    ...
    def test_a_configured_contract_that_requires_nothing_is_refused(self):
        with tempfile.TemporaryDirectory() as directory:
            configured = policy(
                directory,
                {"deploy:production": "contracts/deploy-production.json"},
                {"contracts/deploy-production.json": {"requires": []}},
            )
...
class HoldsItsInvariantsWhenConstructedDirectly(unittest.TestCase):
    ...
    def test_registry_must_be_a_map(self):
        for contracts in (None, "deploy:production", ["deploy:production"]):
            with self.subTest(contracts=contracts):
                with self.assertRaises(LawmanError):
                    ContractRegistry(contracts=contracts)

    def test_every_key_is_exactly_action_and_target(self):
        for key in ("deploy", "deploy:production:now", "deploy:", ":production", " : ", ""):
            with self.subTest(key=key):
                with self.assertRaises(LawmanError):
                    ContractRegistry(contracts={key: "contracts/deploy-production.json"})
    ...
    def test_an_unconfigured_intent_has_no_path(self):
        registry = ContractRegistry({"deploy:production": "contracts/deploy-production.json"})

        with self.assertRaises(LawmanError):
            registry.path_for(Intent(action="deploy", target="staging"))
    ...
    def test_the_registry_does_not_alias_the_caller_s_map(self):
        contracts = {"deploy:production": "contracts/deploy-production.json"}
