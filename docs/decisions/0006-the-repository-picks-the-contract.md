...
- **The key is the intent.** `action:target`, the pair Lawman already had. No separate intent ID, because nothing yet needs to distinguish two `deploy -> production` requests, and an ID the requester supplies would be one more thing to trust.
...
`action:target` as a flat string key rather than a nested map because it reads the way people say it. The cost is a delimiter: registry keys must split into exactly two non-blank parts, so `a:b:c` is rejected as malformed rather than resolved ambiguously, and an intent that would produce that key finds nothing and fails closed.
